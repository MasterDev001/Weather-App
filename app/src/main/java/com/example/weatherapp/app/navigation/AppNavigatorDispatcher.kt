...
class AppNavigatorDispatcher @Inject constructor() : AppNavigator, AppNavigatorHandler {
    ...
    override suspend fun navigateTo(screens: List<AppScreen>) = navigator { push(screens) }

    override suspend fun replace(screen: AppScreen) = navigator { replace(screen) }

    override suspend fun replaceAll(screen: AppScreen) = navigator { replaceAll(screen) }

    override suspend fun back() = navigator { pop() }
